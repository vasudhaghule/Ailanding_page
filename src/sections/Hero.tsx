  ...
  const backgroundPositionY=useTransform(scrollYProgress,[0,1],[-300,300]);
    ...
    >
      ...
      <motion.div
        ...
        animate={{
          rotate: [0, 360], // Updated animation
        }}
        transition={{
          repeat: Infinity,
          duration: 20, // More noticeable
        }}
        className="absolute h-[344px] w-[344px] md:h-[580px] md:w-[580px] border border-white/10 rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
      >
        {/* Top Dot */}
        <div className="absolute h-2 w-2 bg-white opacity-20 rounded-full top-0 left-1/2 -translate-x-1/2 -translate-y-1/2"></div>
        {/* Left Dot */}
        <div className="absolute h-2 w-2 bg-white opacity-20 rounded-full top-1/2 left-0 -translate-y-1/2 -translate-x-1/2"></div>
        {/* Right Dot */}
